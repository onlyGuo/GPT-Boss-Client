package com.guoshengkai.gpt.cn.core.dao.impl;

import com.guoshengkai.gpt.cn.core.JSON;
import com.guoshengkai.gpt.cn.core.annotation.po.JsonField;
import com.guoshengkai.gpt.cn.core.beans.*;
import com.guoshengkai.gpt.cn.core.dao.BaseDao;
import com.guoshengkai.gpt.cn.core.sql.where.C;
import com.guoshengkai.gpt.cn.core.sql.where.SqlUtil;
import com.guoshengkai.gpt.cn.core.util.GenericsUtils;
import com.guoshengkai.gpt.cn.core.util.hibernate.jdbc.util.FormatStyle;
import com.guoshengkai.gpt.cn.core.util.lambda.LambdaUtil;
import com.guoshengkai.gpt.cn.exception.ServiceInvokeException;
import com.guoshengkai.gpt.cn.util.DBUtil;
...
public class BaseDaoImpl<T extends PO, PK extends Serializable> implements BaseDao<T, PK> {
	...
	public void initTable(){
		initTable((s) -> {logger.info("SQL => " + s);});
	}

	//	@PostConstruct
	public void initTable(InitTableCall call) {
		// 检查表是否存在
		...
		if (dbUtil.tableExists(tableName)){
			...
			for (Pram pram : sqlParms) {
				if (!dbUtil.columnExists(tableName, pram.getDbField())){
					String sql = "ALTER TABLE " + tableName + " ADD " + pram.getDbField() + " " + pram.getDbType() + " NULL";
					call.call(sql);
					try {
						dbUtil.execute(sql);
					}catch (Exception e){
